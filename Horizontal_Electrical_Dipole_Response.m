...
function [step_ex01,step_ey01,impulse_hx,impulse_hy,impulse_hz] = Horizontal_Electrical_Dipole_Response(u0,rou,m,xr,yr,zr,Tob)
if zr ~= 0
    zr = 0;
end
r = (xr.^2+yr.^2).^0.5;
...
% 脉冲响应
impulse_hx = hx_coff.*((4+temp.^2).*I1-temp.^2.*I0);
...
save(['Horizontal_Electrical_Dipole_Response_rou' num2str(rou) '_xr' num2str(xr) '_yr' num2str(yr) '_zr' num2str(zr) '.mat'],...
    'step_ex01','step_ey01','impulse_hx','impulse_hy','impulse_hz');
